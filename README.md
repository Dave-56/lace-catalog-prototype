...
Then open `http://localhost:3000`. Pick a sample or choose Start with a photo, drag over one item, and press Search. The frontend calls `POST /api/catalog-search`, which proxies the selected image crop to Shopify Global Catalog `search_catalog` using `catalog.like`.
